...
function ShaderBackground() {
  ...
  return (
    <div ref={canvasRef} className="absolute inset-0 -z-10 h-full w-full bg-black" aria-hidden>
      <Canvas camera={camera} gl={{ antialias: true, alpha: false }} dpr={[1, 2]} className="h-full w-full">
        <ShaderPlane />
      </Canvas>
...
export default function NeuralNetworkHero({
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 bg-black/20" />
    </section>
  )
}
