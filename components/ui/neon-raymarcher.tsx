...
const createInitialState = (amount: number): AnimationState => ({
  ...
  baseOffsets: Array.from({ length: amount }, (_, i) => {
    ...
    return {
      x: Math.cos(t) * 1.75,
      y: Math.sin(t) * 4.5,
      posSpeed: new THREE.Vector3(1 + Math.random() * 4, 1 + Math.random() * 3.5, 0.5 + Math.random() * 2),
      rotSpeed: new THREE.Vector3(0.1 + Math.random(), 0.1 + Math.random(), 0.1 + Math.random()),
      posPhase: new THREE.Vector3(
        t + Math.random() * Math.PI * 3,
        t * 1.3 + Math.random() * Math.PI * 3,
        t * 0.7 + Math.random() * Math.PI * 3,
      ),
      rotPhase: new THREE.Vector3(
        t * 0.5 + Math.random() * Math.PI * 2,
        t * 0.8 + Math.random() * Math.PI * 2,
        t * 1.1 + Math.random() * Math.PI * 2,
      ),
    }
  }),
})
