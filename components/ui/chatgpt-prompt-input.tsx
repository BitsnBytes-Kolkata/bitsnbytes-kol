...
export const PromptBox = React.forwardRef<PromptBoxRef, PromptBoxProps>(
    ({ className, onSubmitMessage, value: externalValue, onChange: externalOnChange, ...props }, ref) => {
        ...
        React.useImperativeHandle(ref, () => ({
            ...
            setValue: (val: string) => {
                if (externalOnChange) {
                    // Pseudo-event since we can't easily construct a ChangeEvent
                    const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, "value")?.set;
                    if (nativeInputValueSetter && internalTextareaRef.current) {
                        nativeInputValueSetter.call(internalTextareaRef.current, val);
