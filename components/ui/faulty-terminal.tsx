...
float digit(vec2 p){
    ...
    float glyphBrightness = isOn * (0.2 + y * 0.8) * (0.75 + x * 0.25);

    return step(0.0, p.x) * step(p.x, 1.0) * step(0.0, p.y) * step(p.y, 1.0) * glyphBrightness;
}
  ...
  useEffect(() => {
    ...
    const update = (timeMs: number) => {
      ...
      if (pageLoadAnimation && loadAnimationStartRef.current > 0) {
        ...
        program.uniforms.uPageLoadProgress.value = Math.min(animationElapsed / animationDurationMs, 1);
      }
